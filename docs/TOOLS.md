...
| **git**              | Sandboxed Git operations: `status`, `diff`, `log`, `show`, `blame`. Target paths must resolve inside the repository root.                                                                                                                                     |
| **websearch**        | Web search via DuckDuckGo; returns titles, URLs, and snippets.                                                                                                                                                                                                |
| **fetch**            | Fetch and extract readable text from a web page. Uses BeautifulSoup with user-agent rotation, rejects non-textual content types. Supports `include_links` and `include_images`.                                                                               |
| **extract**          | Fetch and extract readable text from a web page in Markdown format. Uses DDGS.                                                                                                                                                                                |
...
| **search_google**    | Search Google (scraped via googlesearch-python); returns titles, URLs, and descriptions.                                                                                                                                                                      |
| **search_stackoverflow** | Search Stack Overflow questions; returns titles, URLs, and scores, optionally filtered by tag.                                                                                                                                                            |
| **search_bing**      | Search Bing (via search-engine-parser); returns titles and URLs.                                                                                                                                                                                               |
| **search_open_library** | Search the Open Library catalog for books; returns titles, authors, first publication year, and URLs.                                                                                                                                                         |
| **search_hacker_news** | Search Hacker News stories by keyword (Algolia API); returns titles, URLs, and points.                                                                                                                                                                      |
| **top_hacker_news_stories** | Fetch the current top stories on Hacker News; returns titles and URLs.                                                                                                                                                                                  |
| **search_gnews**     | Search Google News for recent articles; returns titles, URLs, and published dates.                                                                                                                                                                             |
...
- **Read/shell tools** (`read_file`, `view_image`, `glob`, `grep`, `list_directory`, `tree`, `run_command`, `git`) -- prompt when targeting paths outside the workspace. If no workspace is set (`/workspace unset`), these always prompt.
...
- **Unrestricted tools** -- `websearch`, `search_wikipedia`, `search_arxiv`, `search_google`, `search_stackoverflow`, `search_bing`, `search_open_library`, `search_hacker_news`, `top_hacker_news_stories`, `search_gnews`, `think`, `todowrite`, `notebook` -- no permission gating (the network ones are still blocked by offline mode).
...
- **Shell allowlist** -- `run_command` only executes binaries in `ALLOWED_COMMANDS`, composed by unioning grouped frozensets (`_ALLOWED_FILESYSTEM_READ`, `_ALLOWED_TEXT_SEARCH`, `_ALLOWED_TEXT_PROCESSING`, `_ALLOWED_ENCODING_COMPRESSION`, `_ALLOWED_UTILITIES`, `_ALLOWED_SYSTEM_INFO`, `_ALLOWED_RUNTIME_COMMANDS`) in `tools/shell.py`. Pipes (`|`) and logical operators (`&&`, `||`, `;`) are allowed and safe because each segment is independently validated. `xargs` is allowlisted but its **inner** command is validated recursively — `xargs rm` still fails. `git` is excluded; use the dedicated `git` tool (restricted to `status`/`diff`/`log`/`show`/`blame` and pinned to the repo root). Once an interpreter is allowlisted, the sandbox for that binary is advisory — `python -c '…'`, `node -e '…'`, `pytest` fixtures etc. can execute arbitrary code by design.
...
When enabled (via `/config`, `OLI_DRY_RUN=true`, or `--dry-run`), destructive tools (`write_file`, `edit_file`, `download_file`, `upload_file`, `run_command`, `git`) return a preview of what would execute without actually running. A `[DRY RUN]` banner appears in the header and status bar.

## Offline mode

Enabled by default. Blocks all network access for web tools (`websearch`, `fetch`, `search_wikipedia`, `search_github`, `search_arxiv`, `search_google`, `search_stackoverflow`, `search_bing`, `search_open_library`, `search_hacker_news`, `top_hacker_news_stories`, `search_gnews`, `extract_article`) and HTTP-based MCP servers. Toggle at runtime via `/offline` or `/config`. Disable on startup with `--no-offline` or `OLI_OFFLINE_MODE=false`.
